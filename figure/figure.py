...
"""
Wan2.1 1.3B Text-to-Video (T2V) 模型架构 —— 用 TensorBoard 可视化计算图。

本脚本把 DiT 主干 (WanModel) 的计算图写入 TensorBoard 的 event 文件，并把
每个输入 / 输出张量的 shape 一并记录（既写进 TensorBoard 的 TEXT 面板，也写进
figure/wan_t2v_1_3B_io_shapes.txt）。

运行:
    python figure/figure.py                     # 默认 2 个 transformer block, 便于看清结构
    python figure/figure.py --layers 4          # 自定义 block 数
    python figure/figure.py --full              # 完整 30 层 (需要较大内存, ~6GB)

查看:
    tensorboard --logdir figure/runs
    然后在浏览器打开 http://localhost:6006 的 GRAPHS 面板
...
def _sdpa_attention(q, k, v, *args, **kwargs):
    ...
    这里忽略变长 mask (q_lens/k_lens) 与 window_size —— 它们只影响数值, 不影响
    张量 shape 与网络拓扑, 对“看架构”没有影响。
    """
...
class TraceableWanT2V(torch.nn.Module):
...
def main():
    parser = argparse.ArgumentParser(description='用 TensorBoard 可视化 Wan2.1 1.3B T2V 架构')
    parser.add_argument('--layers', type=int, default=2,
                        help='可视化用的 transformer block 数 (默认 2, 图更清晰)')
    parser.add_argument('--full', action='store_true',
                        help='使用完整 30 层 (需要较大内存)')
    parser.add_argument('--logdir', type=str, default=os.path.join(_OUT_DIR, 'runs'),
    ...
    # 关键: 用 CPU 友好实现替换注意力与 rope, 使模型可在 CPU 上被 trace
    ...
    model = TraceableWanT2V(base_model)
    model.eval()
    ...
    txt_path = os.path.join(_OUT_DIR, 'wan_t2v_1_3B_io_shapes.txt')
